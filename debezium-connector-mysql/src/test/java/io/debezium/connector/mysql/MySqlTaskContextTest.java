...
public class MySqlTaskContextTest {
    ...
    @Test
    public  void testDDLFilter() throws Exception
    {
        config = simpleConfig().with(MySqlConnectorConfig.SNAPSHOT_MODE, SnapshotMode.NEVER)
                .with("internal.database.history.ddl.filter","DROP TEMPORARY TABLE IF EXISTS .+ /\\\\* generated by server \\\\*/" +
                        ",INSERT INTO mysql.rds_heartbeat2\\\\(.*\\\\) values \\\\(.*\\\\) ON DUPLICATE KEY UPDATE value = .*,FLUSH RELAY LOGS.*,flush relay logs.*,SAVEPOINT .*,DELETE from mysql\\..*")
                .build();
